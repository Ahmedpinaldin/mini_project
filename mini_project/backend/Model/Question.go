...
func GetQuestionsByUser(db *sql.DB) ([]Question, error) {
	var questions []Question
	rows, err := db.Query("SELECT id, pertanyaan, id_quiz FROM pertanyaan")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	for rows.Next() {
		var question Question
		if err := rows.Scan(&question.ID, &question.Pertanyaan, &question.QuizID); err != nil {
			return nil, err
		}
		questions = append(questions, question)
	}
